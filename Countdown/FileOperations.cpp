...
void CFileOperation::DoCopy(CString sSource, CString sDest, bool bDelteAfterCopy)
{
	...
	if (CheckPath(sSource) == PATH_IS_FILE && CheckPath(sDest) == PATH_IS_FOLDER) 
	{
		...
		_splitpath_s(sSource, drive, sizeof(drive), dir, sizeof(dir), name, sizeof(name), ext, sizeof(ext));
...
CString CFileOperation::ChangeFileName(CString sFileName)
{
	...
	char name [MAX_PATH];
	char ext  [MAX_PATH];
	_splitpath_s((LPCTSTR)sFileName, drive, sizeof(drive), dir, sizeof(dir), name, sizeof(name), ext, sizeof(ext));
	sName = name;
