...
int CHighTime::FindStr(LPCTSTR pszSource, LPCTSTR pszSub, int nStart)
{
	...
	// find first
	auto pszSearch = strstr(pszSource + nStart, pszSub);
...
LPCTSTR CHighTime::Format(LPSTR pBuffer, int iBufferLen, LPCTSTR pFormat) const
{
	...
	{
		strncpy_s(pBuffer, iBufferLen, INVALID_DATETIME, iBufferLen);
		return pBuffer;
	}
    ...
    strncpy_s(pBuffer, iBufferLen, pFormat, iBufferLen);
    ...
    strcpy_s(szTemp, iBufferLen, pBuffer);
...
CString CHighTime::Format(LPCTSTR lpszFormat) const
{
    ...
    if (iPos >= 0) {
        szTemp.Format("%03d",abs((long)tmHighTimeTemp.nMilli));
        szParse.Replace("%s",szTemp);
    }
    iPos = szParse.Find("%#s", 0);
    if (iPos >= 0) {
        szTemp.Format("%d",abs((long)tmHighTimeTemp.nMilli));
        szParse.Replace("%#s",szTemp);
    }
    iPos = szParse.Find("%u", 0);
    if (iPos >= 0) {
        szTemp.Format("%03d",abs((long)tmHighTimeTemp.nMicro));
        szParse.Replace("%u",szTemp);
    }
    iPos = szParse.Find("%#u", 0);
    if (iPos >= 0) {
        szTemp.Format("%d",abs((long)tmHighTimeTemp.nMicro));
        szParse.Replace("%#u",szTemp);
    }
    iPos = szParse.Find("%n", 0);
    if (iPos >= 0) {
        szTemp.Format("%03d",abs((long)tmHighTimeTemp.nNano));
        szParse.Replace("%n",szTemp);
    }
    iPos = szParse.Find("%#n", 0);
    if (iPos >= 0) {
        szTemp.Format("%d",abs((long)tmHighTimeTemp.nNano));
        szParse.Replace("%#n",szTemp);
    }
